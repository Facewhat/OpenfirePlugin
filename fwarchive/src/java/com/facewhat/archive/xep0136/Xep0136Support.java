...
public class Xep0136Support
{
    ...
    final Map<String, IQHandler> handlersMap;
    ...
    {
        this.server = server;
        this.handlersMap = Collections.synchronizedMap(new HashMap<String, IQHandler>());
        ...
        this.iqDispatcher = new AbstractIQHandler("XEP-0136 IQ Dispatcher", null) {
            ...
            public IQ handleIQ(IQ packet) throws UnauthorizedException
            {
                ...
                final IQHandler iqHandler = handlersMap.get(packet.getChildElement().getName());
                if (iqHandler != null)
                {
    ...
    public void start()
    {
        for (IQHandler iqHandler : iqHandlers)
        {
            ...
            handlersMap.put(iqHandler.getInfo().getName(), iqHandler);
        ...
        // 都通过它去听，然后在他内部再分发给的具体IQHandler去处理
    ...
    public void stop()
    {
        ...
        for (IQHandler iqHandler : iqHandlers)
        {
            handlersMap.remove(iqHandler.getInfo().getName());
            try
            {
                iqHandler.stop();
                iqHandler.destroy();
